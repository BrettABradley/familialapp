...
BEGIN
  ...
  -- Check if claimer's plan tier can accommodate the circle's current member count
  SELECT COUNT(*) INTO v_member_count
  FROM circle_memberships
  WHERE circle_id = _circle_id;

  SELECT COALESCE(max_members_per_circle, 8) INTO v_max_members
  FROM user_plans
  WHERE user_id = auth.uid();
  ...
  SELECT COALESCE(extra_members, 0) INTO v_circle_extra
  FROM circles
  WHERE id = _circle_id;

  -- Total allowed = plan base + circle extras. The member count from circle_memberships
  -- includes non-owner members. The new owner will also take a slot, so we need
  -- member_count (which includes the claimer already since they're a member) to fit
  -- within max_members (which includes owner).
